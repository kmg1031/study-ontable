...
```sql
CREATE DATABASE IF NOT EXISTS study_ontable
...
```bash
# 데이터베이스 생성
mysql -u root -p -e "CREATE DATABASE IF NOT EXISTS study_ontable DEFAULT CHARACTER SET utf8mb4 DEFAULT COLLATE utf8mb4_unicode_ci;"

# 스키마 파일 실행
mysql -u root -p study_ontable < server/database/schema.sql
```

### 또는 MySQL Workbench 사용
1. MySQL Workbench 실행
2. 위의 쿼리들을 순서대로 복사하여 실행
3. 각 섹션별로 실행하여 오류 확인
